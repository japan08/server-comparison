...
async def get_recommendations(
    ...
    except (ConnectionRefusedError, OSError, DBAPIError) as exc:
        # Keep the API responsive when the configured database is unavailable.
        await session.rollback()
        logger.warning("Recommendation lookup failed because the database is unavailable: %s", exc)
        return []
    rows = result.all()
    return [
        {
