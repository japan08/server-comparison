...
async def get_recommendations(
    ...
    try:
        ...
        # Keep the API responsive when the local database is unavailable.
        logger.warning("Recommendation query failed because the database is unavailable.", exc_info=True)
        return []
    rows = result.all()
    return [
        {
