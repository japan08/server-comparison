...
async def get_recommendations(
    ...
    try:
        ...
        # Keep the API response stable if the backing database is temporarily unavailable.
        logger.warning("Failed to load recommendations from the database", exc_info=True)
        return []
    rows = result.all()
    return [
        {
